...
public class TimKiemLTHDT {
	...
	public static void main(String[] args) {
		...
		System.out.println("--------------------------------");
		// buoc3 tim sinh vien co tuoi lon nhat
		...
		int vitri = Collections.binarySearch(dssv, new SinhVien("Binh", 0, 0), new Comparator<SinhVien>() {
		...
		System.out.println("--------------------------------");
		// buoc 5 su dung Stream va cu phap Lampda
		List<SinhVien> kqloc = dssv.stream().filter(sv -> sv.getDtb() >= 8).filter(sv -> sv.getDtb() <= 10)
				.filter(sv -> sv.getTuoi() >= 18).filter(sv -> sv.getTuoi() <= 20).toList();
		...
		List<SinhVien> dskq = dssv.stream().filter(dk_dtb.or(dk_tuoi)).toList();
